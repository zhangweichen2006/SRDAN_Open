...
VER=pv_rcnn
CONFIG=/home/wzha8158/Dropbox/CVPR2021_LidarPerceptron/Home/OpenLidarPerceptron/tools/cfgs/kitti_models/${VER}.yaml
...
if (($GPUNUM > 1)); then
    ...
    CUDA_VISIBLE_DEVICES=$GPU python -u -m torch.distributed.launch --nproc_per_node=${GPUNUM} tools/traintest.py --launcher pytorch --cfg_file ${CONFIG} --batch_size ${BATCH_SIZE} --epochs $EP --out_dir=$OUT_DIR --test_batch_size $TEST_BATCH_SIZE |& tee -a $OUTPUT_LOG
    ...
    CUDA_VISIBLE_DEVICES=$GPU python -u tools/traintest.py --cfg_file ${CONFIG} --batch_size ${BATCH_SIZE} --epochs $EP --out_dir=$OUT_DIR --test_batch_size $TEST_BATCH_SIZE |& tee -a $OUTPUT_LOG
