...
if (($GPUNUM > 1)); then
    ...
    CUDA_VISIBLE_DEVICES=$GPU python -u -m torch.distributed.launch --nproc_per_node=${GPUNUM} tools/train.py --launcher pytorch --cfg_file ${CONFIG} --batch_size ${BATCH_SIZE} --epochs $EP --tcp_port=${PORT} |& tee  $OUTPUT_LOG
