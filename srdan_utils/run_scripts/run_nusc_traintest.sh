...
if (($GPUNUM > 1)); then
    ...
    CUDA_VISIBLE_DEVICES=$GPU python -u -m torch.distributed.launch --nproc_per_node=${GPUNUM} --master_port=$RANDOM tools/traintest.py --launcher pytorch --cfg_file ${CONFIG} --batch_size ${BATCH_SIZE} --epochs $EP --out_dir=$OUT_DIR --test_batch_size $TEST_BATCH_SIZE --location ${TGT} --trainval ${TRAINVAL} |& tee  $OUTPUT_LOG
