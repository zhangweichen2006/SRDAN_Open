...
void points_in_boxes_launcher_nusc(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *box_idx_of_points);
...
int points_in_boxes_gpu_nusc(at::Tensor boxes_tensor, at::Tensor pts_tensor, at::Tensor box_idx_of_points_tensor){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params boxes_idx_of_points: (B, npoints), default -1
    ...
    points_in_boxes_launcher_nusc(batch_size, boxes_num, pts_num, boxes, pts, box_idx_of_points);
...
int points_in_boxes_cpu_nusc(at::Tensor boxes_tensor, at::Tensor pts_tensor, at::Tensor pts_indices_tensor){
    // params boxes: (N, 9) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center, each box DO NOT overlaps
    // params pts: (num_points, 3) [x, y, z]
    // params pts_indices: (N, num_points)
    ...
    for (int i = 0; i < boxes_num; i++){
        for (int j = 0; j < pts_num; j++){
            int cur_in_flag = check_pt_in_box3d_cpu(pts + j * 3, boxes + i * 9, local_x, local_y);
            pts_indices[i * pts_num + j] = cur_in_flag;
        }
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("points_in_boxes_gpu_nusc", &points_in_boxes_gpu_nusc, "points_in_boxes_gpu_nusc forward (CUDA)");
    m.def("points_in_boxes_cpu", &points_in_boxes_cpu, "points_in_boxes_cpu forward (CUDA)");
    m.def("points_in_boxes_cpu_nusc", &points_in_boxes_cpu_nusc, "points_in_boxes_cpu_nusc forward (CUDA)");
}
