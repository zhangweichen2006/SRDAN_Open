...
def points_in_boxes_cpu(points, boxes, nusc=False):
    """
    Args:
        points: (num_points, 3)
    ...
    # if nusc:
    #     boxes = boxes.float().contiguous()[:,[0,1,2,3,4,5,8]]

    point_indices = points.new_zeros((boxes.shape[0], points.shape[0]), dtype=torch.int)
    if nusc:
        roiaware_pool3d_cuda.points_in_boxes_cpu_nusc(boxes, points.float().contiguous(), point_indices)
    else:
        roiaware_pool3d_cuda.points_in_boxes_cpu(boxes, points.float().contiguous(), point_indices)

    return point_indices.numpy() if is_numpy else point_indices

def points_in_boxes_gpu(points, boxes, nusc=False):
    """
    :param points: (B, M, 3)
    ...
    # if nusc:
    #     boxes = boxes.contiguous()[:,[0,1,2,3,4,5,8]]

    box_idxs_of_pts = points.new_zeros((batch_size, num_points), dtype=torch.int).fill_(-1)
    if nusc:
        roiaware_pool3d_cuda.points_in_boxes_gpu_nusc(boxes, points.contiguous(), box_idxs_of_pts)
    else:
        roiaware_pool3d_cuda.points_in_boxes_gpu(boxes, points.contiguous(), box_idxs_of_pts)

    return box_idxs_of_pts


class RoIAwarePool3d(nn.Module):
    def __init__(self, out_size, max_pts_each_voxel=128, nusc=False):
        super().__init__()
    ...
    def forward(self, rois, pts, pts_feature, pool_method='max'):
        ...
        return RoIAwarePool3dFunction.apply(rois, pts, pts_feature, self.out_size, self.max_pts_each_voxel, pool_method, nusc=self.nusc)


class RoIAwarePool3dFunction(Function):
    @staticmethod
    def forward(ctx, rois, pts, pts_feature, out_size, max_pts_each_voxel, pool_method, nusc=False):
        """
        Args:
            ctx:
        ...
        """
        if nusc:
            assert rois.shape[1] == 9 and pts.shape[1] == 3
        else:
