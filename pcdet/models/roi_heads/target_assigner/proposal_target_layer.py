...
class ProposalTargetLayer(nn.Module):
    def __init__(self, roi_sampler_cfg, nusc=False):
        super().__init__()
        ...
        for index in range(batch_size):
            ...
            if self.roi_sampler_cfg.get('SAMPLE_ROI_BY_EACH_CLASS', False):
                max_overlaps, gt_assignment = self.get_max_iou_with_same_class(
                    ...
                    gt_boxes=cur_gt[:, 0:box_len], gt_labels=cur_gt[:, -1].long(), nusc=self.nusc
                )
            else:
                iou3d = iou3d_nms_utils.boxes_iou3d_gpu(cur_roi, cur_gt[:, 0:box_len])  # (M, N)
    ...
    @staticmethod
    def get_max_iou_with_same_class(rois, roi_labels, gt_boxes, gt_labels, nusc=False):
        """
        Args:
            rois: (N, 7)
            roi_labels: (N)
        ...
        for k in range(gt_labels.min().item(), gt_labels.max().item() + 1):
            ...
            if roi_mask.sum() > 0 and gt_mask.sum() > 0:
                ...
                iou3d = iou3d_nms_utils.boxes_iou3d_gpu(cur_roi, cur_gt, nusc=nusc)  # (M, N)
