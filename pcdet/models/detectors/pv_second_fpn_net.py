...
class PVSECONDFPNNet(Detector3DTemplate):
    ...
    def get_training_loss(self, t_mode='det'):
        disp_dict = {}
        tb_dict = {}
        self.point_seg = self.model_cfg.get('POINT_SEG', True)
        ...
        if 'det' in t_mode or 'pseudo' in t_mode:
            ...
            self.point_da = self.model_cfg.get('POINT_DA', True)
            self.sep_point_da = self.model_cfg.get('SEP_POINT_DA', False)
            if self.point_da:
                if self.sep_point_da:
