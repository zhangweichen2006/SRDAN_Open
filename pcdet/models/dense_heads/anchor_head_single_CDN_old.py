...
class AnchorHeadSingleCDN(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        self.conv_gamma_w = nn.Parameter(torch.randn(1, input_channels, 1), requires_grad=True)
        ...
        self.conv_beta_b = nn.Parameter(torch.randn(1, input_channels, 1), requires_grad=True)
    ...
    def forward(self, data_dict):
        ...
        if 'dom_img' in t_mode:
            ...
            # else:
                # print('self.temp_src_feat', self.temp_src_feat.shape)
                ...
                # print('self.temp_src_emb', self.temp_src_emb.shape)
                # print('conv_gamma_w', self.conv_gamma_w.shape)
                self.src_gamma = torch.mul(self.conv_gamma_w, self.temp_src_emb.unsqueeze(-1)) + self.conv_gamma_b
                # print('self.src_gamma', self.src_gamma.shape)
                self.src_beta = torch.mul(self.conv_beta_w, self.temp_src_emb.unsqueeze(-1)) + self.conv_beta_b
                # print('self.src_beta', self.src_beta.shape)
                x_pool = self.domain_pool(torch.mul(self.src_gamma.unsqueeze(-1), spatial_features_2d) + self.src_beta.unsqueeze(-1).expand_as(spatial_features_2d)).view(spatial_features_2d.size(0), -1)
