...
class AnchorHeadSingleContextFPNWeak(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        if self.model_cfg.get('USE_DOMAIN_CLASSIFIER', None):
            ...
            if self.sep_two_dom:
                ...
                self.conv_dom_layers = LocalDomainClassifier(input_channels=input_channels, context=self.range_guidance_new_conv_dom_context)
    ...
    def forward(self, data_dict):
        ...
        if 'dom_img' in t_mode and not self.fpn_only:
            ...
            # if self.sep_two_dom:
            #     x_reverse_dom_sep = grad_reverse(spatial_features_2d, l)
            #     dom_img_preds, _ = self.conv_dom_layers(x_reverse_dom_sep)

            #     self.forward_ret_dict['dom_img_preds'] = dom_img_preds

            #     _, pixel_context = self.conv_dom_layers(spatial_features_2d.detach())
        ...
        if 'dom_img_det' in t_mode:
            ...
            if not self.fpn_only:
                ...
                if self.sep_two_dom:
                    ...
                    # print("dom_head_context2", dom_head_context2.shape)
                    dom_head_context2 = dom_head_context2.repeat(1,1,spatial_features_2d.shape[-2],spatial_features_2d.shape[-1])

                    dom_head_context_all_reshape = torch.cat((dom_head_context_all_reshape, dom_head_context2),dim=1)
