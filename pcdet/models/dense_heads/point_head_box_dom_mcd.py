...
class PointHeadBoxDomMCD(PointHeadTemplate):
    """
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        if self.mcd:
            self.cls_layers2 = self.make_fc_layers(
                fc_cfg=self.model_cfg.CLS_FC,
                input_channels=input_channels,
                output_channels=num_class
            )
        ...
        if self.mcd:
            self.box_layers2 = self.make_fc_layers(
                fc_cfg=self.model_cfg.REG_FC,
                input_channels=input_channels,
                output_channels=self.box_coder.code_size
            )
    ...
    def get_mcd_src_loss(self, tb_dict={}):
        ...
        tb_dict = {} if tb_dict is None else tb_dict
        point_loss_cls, tb_dict_1 = self.get_cls_layer_loss(mcd_id=1)
        point_loss_box, tb_dict_2 = self.get_box_layer_loss(mcd_id=1)

        point_loss_cls2, tb_dict_12 = self.get_cls_layer_loss(mcd_id=2)
        point_loss_box2, tb_dict_22 = self.get_box_layer_loss(mcd_id=2)

        point_loss = point_loss_cls + point_loss_box + point_loss_cls2 + point_loss_box2
        tb_dict.update(tb_dict_1)
        tb_dict.update(tb_dict_2)
        tb_dict.update(tb_dict_12)
        tb_dict.update(tb_dict_22)
        
        return point_loss, tb_dict

    def get_mcd_tgt_loss(self, tb_dict={}):

        point_mcd_loss_cls, tb_dict_1 = self.get_mcd_cls_layer_loss()
        point_mcd_loss_box, tb_dict_2 = self.get_mcd_box_layer_loss()

        point_loss = point_mcd_loss_cls + point_mcd_loss_box
    ...
    def forward(self, batch_dict):
        ...
        if t_mode == 'dom_img_tgt' and self.mcd:
            prefix = 'mcd_'
            point_features = grad_reverse(point_features, l*-1)
        else:
            prefix = ''
        ...
        if self.mcd:
            ...
            batch_dict[f'{prefix}point_cls_scores_1'] = torch.sigmoid(point_cls_preds_max)

            ret_dict = {f'{prefix}point_cls_preds_1': point_cls_preds,
                        f'{prefix}point_box_preds_1': point_box_preds}

            point_cls_preds2 = self.cls_layers2(point_features)  # (total_points, num_class)
            point_box_preds2 = self.box_layers2(point_features)  # (total_points, box_code_size)

            point_cls_preds_max2, _ = point_cls_preds2.max(dim=-1)
            batch_dict[f'{prefix}point_cls_scores_2'] = torch.sigmoid(point_cls_preds_max2)

            ret_dict.update({f'{prefix}point_cls_preds_2': point_cls_preds2,
                        f'{prefix}point_box_preds_2': point_box_preds2}) 
