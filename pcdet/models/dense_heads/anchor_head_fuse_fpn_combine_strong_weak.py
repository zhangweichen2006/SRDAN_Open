...
class AnchorHeadFuseFPNCombine(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        if self.sep_two_dom or (self.double_pma and not self.joint_pma):
            ...
            print("input_channels_dom_sep", self.input_channels_dom_sep)
        ...
        if self.dom_context:
            dom_fc1, dom_fc2 = self.model_cfg.get('DOM_FC', [1024, 128])
        else:
        ...
        print(" self.input_channels_fpn", self.input_channels_fpn)
        #############
            ...
            if 'dom_img' in t_mode and not self.fpn_only:
                ...
                if self.range_guidance_conv_dom or self.range_guidance_new_conv_dom:
                    ...
                    if self.range_guidance_double_dom:
                        ...
                        x_pool_joint = torch.cat((x_pool,x_pool2),dim=-1)
                    ...
                    if self.sep_two_dom:
                        # print("spatial_features_2d_dom_sep", spatial_features_2d_dom_sep.shape)
                        ...
                        if 'dom_img_det' in t_mode:
                            dom_context2 = self.domain_classifier2[:-2](x_reverse2.detach())#.squeeze(-1)
                            data_dict['dom_head_context_2'] = dom_context2.unsqueeze(-1).unsqueeze(-1)
                    ...
                    for layer in self.fpn_layers:
                        ...
                        if self.range_guidance_conv_dom or self.range_guidance_new_conv_dom:
                            ...
                            if self.sep_two_dom:
                                ...
                                if 'dom_img_det' in t_mode:
                                    dom_context2_fpn = self.domain_classifier2_fpn[layer][:-2](x_reverse2_sep_fpn.detach())#.squeeze(-1)
                                    data_dict[f'dom_head_context_2_fpn{layer}'] = dom_context2_fpn.unsqueeze(-1).unsqueeze(-1)
        ...
        if 'det' in t_mode:
            if not self.fpn_only:
                ...
                if self.range_guidance_new_conv_dom_context:
                    ...
                    dom_head_context = []
                    dom_head_context.append(data_dict['dom_head_context'])
                    # print("dom_head_context", dom_head_context[0].shape)

                    if f'dom_head_context_2' in data_dict:
                        dom_head_context.append(data_dict[f'dom_head_context_2'])
                        # print("dom_head_context", dom_head_context[1].shape)

                    dom_head_context = torch.cat(dom_head_context, dim=1)
                    ...
                    if len(self.fpn_layers) != 0:
                        ...
                        for layer in self.fpn_layers:
                            dom_head_context_fpn.append(data_dict[f'dom_head_context_fpn{layer}'])
                            if f'dom_head_context_2_fpn{layer}' in data_dict:
                                dom_head_context_fpn.append(data_dict[f'dom_head_context_2_fpn{layer}'])

                        if self.sep_fpn_dom_context:
                            dom_head_context_all = dom_head_context_fpn#torch.cat((dom_head_context_all, dom_head_context), dim=1)
                        else:
                            dom_head_context_all = torch.cat(dom_head_context_fpn, dim=1)
                            dom_head_context_all = torch.cat((dom_head_context_all, dom_head_context_fpn), dim=1) #dom_point_context
                    else:
            ...
            if self.num_fpn_up + self.num_fpn_down + self.num_fpn_downup > 0:
                ...
                for layer in self.fpn_layers:
                    ...
                    if self.range_guidance_new_conv_dom_context:
                        ...
                        print('dom_head_context_all_fpn_reshape', dom_head_context_all_fpn_reshape.shape)
