...
class AnchorHeadFuseFPNCombineSelfAttention(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        if self.voxel_dom_patch_attention:
            ...
            if self.double_pma:
                if self.joint_pma:
                    self.att_patch_layer_double = Self_Attn(self.input_channels_dom_joint)
                else:
                    self.att_patch_layer_double = Self_Attn(self.input_channels_dom)

            if self.joint_two_dom:

                if self.two_attention_max:
                    self.att_patch_layer = Self_Attn(512)
                else:
                    self.att_patch_layer = Self_Attn(512)
            else:
                if self.two_attention_max:
