    ...
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        if self.range_guidance:
            if self.fov:
                ...
                if self.model_cfg.get('NEG_HORIZONTAL_LOC', False):
                    self.y_range_matrix = (torch.arange(-half_range_x, half_range_x, 1).float() + 0.5).unsqueeze(-1).unsqueeze(0).unsqueeze(0).repeat(1,1,1,total_range_y).cuda()
                else:
                ...
                if self.model_cfg.get('NEG_VERTICAL_LOC', False):
                    self.x_range_matrix = (torch.arange(-half_range_y, half_range_y, 1).float() + 0.5).unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(1,1, total_range_x, 1).cuda()
                else:
            ...
            for layer in self.fpn_layers:
                if self.fov:
                    ...
                    # print('x_range', x_range)
                    if self.model_cfg.get('NEG_HORIZONTAL_LOC', False):
                        self.y_range_matrix_fpn[layer] = (torch.arange(-half_range_x, half_range_x, 1).float() + 0.5).unsqueeze(-1).unsqueeze(0).unsqueeze(0).repeat(1,1,1,total_range_y).cuda()
                    else:
                    ...
                    half_range_y = int(total_range_y * 0.5)
                    if self.model_cfg.get('NEG_VERTICAL_LOC', False):
                        self.x_range_matrix_fpn[layer] = (torch.arange(-half_range_y, half_range_y, 1).float() + 0.5).unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(1,1, total_range_x, 1).cuda()
                    else:
