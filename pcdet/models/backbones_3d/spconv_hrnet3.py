...
class CatTensors(spconv.SparseModule):
    def forward(self, x: spconv.SparseConvTensor, x2: spconv.SparseConvTensor):
        ...
        # new_perm = torch.zeros(indices_index_len).long().cuda()
        # for i in range(indices_index_len):
        #     if flag[i]:
        #         new_id = perm[i]
        #     new_perm[i] = new_id
        ...
        new_features = features_cat[new_indices_index]
        new_inds = inds_joint[new_indices_index]
        ...
        # res.indice_dict = {**x2.indice_dict, **x.indice_dict}
...
class HRNet3(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, num_fpn_up=0, num_fpn_downup=0, **kwargs):
        ...
        self.partialm = False#self.model_cfg.get('NOM', False)
        self.nom = False#self.model_cfg.get('NOM', False)
        self.allsp = False
        ...
        #             spconv.SparseConv3d(input_channels, 16, 3, padding=1,bias=False, indice_key='conv_input'),norm_fn(16),
        #             nn.ReLU(),)
        # else:
        self.conv_input = spconv.SparseSequential(
                spconv.SubMConv3d(input_channels, 16, 3, padding=1, bias=False, indice_key='conv_input'),
                norm_fn(16),
                nn.ReLU(),)

        # layer 1
        self.conv1 = spconv.SparseSequential(
            block(16, 16, 3, norm_fn=norm_fn, padding=1, indice_key='conv1', conv_type='subm'), 
        )
        self.conv1_transition_1_1 = spconv.SparseSequential(
            block(16, 16, 3, norm_fn=norm_fn, stride=1, padding=1, indice_key='conv1_transition_1_1', conv_type='spconv'),
        )
        self.conv1_transition_1_2 = spconv.SparseSequential(
            block(16, 32, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='conv1_transition_1_2', conv_type='spconv'),
        )

        # layer 2 (stage)
        self.conv2_1 = spconv.SparseSequential(
            block(16, 16, 3, norm_fn=norm_fn, padding=1, indice_key='conv2_1', conv_type='subm'),
            # block(16, 16, 3, norm_fn=norm_fn, padding=1, indice_key='subm2_12'),
        )
        self.conv2_2 = spconv.SparseSequential(
            block(32, 32, 3, norm_fn=norm_fn, padding=1, indice_key='conv2_21',conv_type='spconv'),
            block(32, 32, 3, norm_fn=norm_fn, padding=1, indice_key='conv2_22',conv_type='subm'),
        )
        self.conv2_transition_down_1 = spconv.SparseSequential(
            block(16, 32, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='conv2_transition_down_1', conv_type='spconv', no_relu=True),
        )
        self.conv2_transition_down_2 = spconv.SparseSequential(
            block(32, 64, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='conv2_transition_down_2', conv_type='spconv', no_relu=True),
        )
        ...
        #     block(32, 16, 3, norm_fn=norm_fn, indice_key='conv1_transition_1_2', conv_type='inverseconv', no_relu=True)
        # ) # can use conv and upsample 
        
        # layer 3 (stage)
        self.conv3_1 = spconv.SparseSequential(
            # [800, 704, 21] <- [400, 352, 11]
            block(16, 16, 3, norm_fn=norm_fn, padding=1, indice_key='conv3_1', conv_type='subm'),
            # block(16, 16, 3, norm_fn=norm_fn, padding=1, indice_key='subm3_12'),
        )
        self.conv3_2 = spconv.SparseSequential(
            # [800, 704, 21] <- [400, 352, 11]
            block(32, 32, 3, norm_fn=norm_fn, padding=1, indice_key='conv3_2', conv_type='subm'),
            # block(32, 32, 3, norm_fn=norm_fn, padding=1, indice_key='subm3_22'),
        )
        self.conv3_3 = spconv.SparseSequential(
            # [800, 704, 21] <- [400, 352, 11]
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='conv3_31', conv_type='spconv'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='conv3_32', conv_type='subm'),
        )
        self.conv3_transition_down_1 = spconv.SparseSequential(
            block(16, 32, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='conv3_transition_down_1', conv_type='spconv', no_relu=True),
        )
        self.conv3_transition_down_2 = spconv.SparseSequential(
            block(32, 64, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='conv3_transition_down_2', conv_type='spconv', no_relu=True),
        )
        self.conv3_transition_down_3 = spconv.SparseSequential(
            block(64, 64, 3, norm_fn=norm_fn, stride=2, padding=(0, 1, 1), indice_key='conv3_transition_down_3', conv_type='spconv', no_relu=True),
        )
        # self.conv3_transition_up_1 = spconv.SparseSequential(
        #     block(32, 16, 3, norm_fn=norm_fn, indice_key='conv2_transition_down_1', conv_type='inverseconv', no_relu=True)
        # )
        # self.conv3_transition_up_2 = spconv.SparseSequential(
        #     block(64, 32, 3, norm_fn=norm_fn, indice_key='conv2_transition_down_2', conv_type='inverseconv', no_relu=True)
        # )  # can use conv and upsample 
        ...
        self.conv4_1 = spconv.SparseSequential(
            ...
            block(16, 16, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_11', conv_type='subm'),
            # block(16, 16, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_12'),
        )
        self.conv4_2 = spconv.SparseSequential(
            # [400, 352, 11] <- [200, 176, 5]
            # block(32, 32, 3, norm_fn=norm_fn, padding=1, indice_key='spconv4_2', conv_type='spconv'),
            block(32, 32, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_21', conv_type='subm'),
            # block(32, 32, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_22'),
        )
        self.conv4_3 = spconv.SparseSequential(
            # [400, 352, 11] <- [200, 176, 5]
            # block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='spconv4_3', conv_type='spconv'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_31', conv_type='subm'),
            # block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_32'),
        )
        self.conv4_4 = spconv.SparseSequential(
            # [400, 352, 11] <- [200, 176, 5]
            # block(128, 128, 3, norm_fn=norm_fn, padding=1, indice_key='spconv4_4', conv_type='spconv'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_41', conv_type='spconv'),
            block(64, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm4_42', conv_type='subm'),
        )
        
        self.conv4_transition_down_1 = spconv.SparseSequential(
            block(16, 32, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv3_down123_1', conv_type='spconv', no_relu=True),
        )
        self.conv4_transition_down_2 = spconv.SparseSequential(
        ...
        self.conv4_transition_down_3 = spconv.SparseSequential(
            block(64, 64, 3, norm_fn=norm_fn, stride=2, padding=(0, 1, 1), indice_key='spconv3_down3', conv_type='spconv', no_relu=True),
        )
        # block(32, 64, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv3_down123_2', conv_type='spconv', no_relu=True),
        # block(64, 64, 3, norm_fn=norm_fn, stride=2, padding=(0, 1, 1), indice_key='spconv3_down123_3', conv_type='spconv', no_relu=True),
        # block(64, 64, 3, norm_fn=norm_fn, stride=2, padding=(0, 1, 1), indice_key='spconv3_down23_3', conv_type='spconv', no_relu=True),
    ...
    #             # conv3: 1, 64, 11, 252, 252 to 128, 5, 252, 252
                self.conv3_out = spconv.SparseSequential(
                    spconv.SparseConv3d(64, 128, (3, 1, 1), stride=(2, 1, 1), padding=last_pad, bias=False, indice_key='spconv_down2_3'),
                    # 128, 5, 252, 252 -> 640, 252, 252
                    norm_fn(128),
                    nn.ReLU(),
                )
    ...
    def hr_fusion(self, hr_x, hr_modules, hr_layer_in, hr_layer_out):
        ...
        for i in range(hr_layer_in): # 0,1: 2-1
            for j in range(hr_layer_out):# 0,1,2: 2-2
                ...
                if hr_modules[idx_out] == ['ident']:
                    x_out = hr_x[i]
                    # print("x_out", x_out.dense().shape)
                # else:
                elif len(hr_modules[idx_out]) != 0:#i == j - 1 or i == j + 1:
        ...
        for j in range(hr_layer_out): # 012, 0123
            # print('layer', j)
            in_append = False
            for i in range(hr_layer_in): # 01, 012
                idx_hr_next_layer_in = i*hr_layer_out + j #i*hr_layer_out + j
                # print("hr_out[idx_hr_next_layer_in]", hr_out[idx_hr_next_layer_in])
                # print('fuse ij', i,j)
                if i == 0:
    ...
    def forward(self, batch_dict):
        ...
        # print("x_conv2_1", x_conv2_1.dense().shape)
        # print("x_conv2_1", x_conv2_1.dense().shape)

        x_trans_conv2 = [x_conv2_1, x_conv2_2] #[x_conv2_1, x_conv2_2]
        hr_trans_conv2 = [['ident'], [self.conv2_transition_down_1], \
                          [],\
                          [],\
                          ['ident'], [self.conv2_transition_down_2]] #self.conv2_transition_up_1
        ...
        hr_trans_conv3 = [['ident'], [self.conv3_transition_down_1], \
                          [],\
                          [],\
                          [], ['ident'], \
                          [self.conv3_transition_down_2], [], \
                          [], [], \
                          ['ident'], [self.conv3_transition_down_3]] # self.conv3_transition_up_1, self.conv3_transition_up_2
        ...
        x_conv4_1_out = self.conv4_transition_down_1(x_conv4_1)
        x_conv4_2 = self.cat_tensors(x_conv4_1_out, x_conv4_2)

        x_conv4_2_out = self.conv4_transition_down_2(x_conv4_2)
        x_conv4_3 = self.cat_tensors(x_conv4_2_out, x_conv4_3)

        x_conv4_3_out = self.conv4_transition_down_3(x_conv4_3)
        x_conv4_4_out = self.cat_tensors(x_conv4_3_out, x_conv4_4)
        ...
        if self.conv_out is not None:
            ...
            if self.FPN:
                if self.num_fpn_up > 0:
                    ...
                    out_3 = self.conv3_out(x_conv4_3)
