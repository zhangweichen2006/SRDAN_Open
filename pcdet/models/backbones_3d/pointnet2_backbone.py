...
class PointNet2MSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        input_channels = 4
        # print("input_channels new", input_channels)
    ...
    def break_up_pc(self, pc):
        ...
        features = (pc[:, 4].unsqueeze(-1).contiguous() if pc.size(-1) > 4 else None)#.permute(0, 2, 1)().contiguous()
    ...
    def forward(self, batch_dict):
        ...
        # print("features before", features.shape)# 16384, 1 # 16384, 2
        features = features.view(batch_size, -1, features.shape[-1]).permute(0, 2, 1) if features is not None else None # 1, 16384, 2
        # print("features after", features.shape) # 1,2,16384

        l_xyz, l_features = [xyz], [features]
        # print("l_xyz", l_xyz[0].shape)
        # print("l_features", l_features[0].shape)
        # print("SA_modules", self.SA_modules)
        for i in range(len(self.SA_modules)):
            li_xyz, li_features = self.SA_modules[i](l_xyz[i], l_features[i])
            l_xyz.append(li_xyz)
            l_features.append(li_features)

        # print("l_xyz after", len(l_xyz))
        # print("l_features after", len(l_features))
        # print("l_features after", l_features[-1].shape)
        ...
        # print("l_features last", len(l_features))
        # print("l_features last", l_features[-1].shape)
        ...
        # print("batch_dict['point_coords']", batch_dict['point_coords'].shape)
        return batch_dict
